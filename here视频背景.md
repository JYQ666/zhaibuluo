Build a "CTA + Footer" section component for a React + Vite + Tailwind CSS project. This is a cinematic full-width call-to-action section with an MP4 video background, centered text, two CTA buttons, and a minimal footer bar at the bottom. Black background, white text, liquid glassmorphism effects.
...
- No video library needed -- MP4 plays natively in all modern browsers
...
Local MP4 file (H.264, 1264x720, 30fps, ~30s loop). Plays natively via `<video>` tag -- no HLS library required. Place the file in the `public/videos/` directory of the Vite project so it's served at `/videos/hero.mp4`.
...
### MP4 VIDEO SETUP PATTERN

The `<video>` element uses native browser playback -- no JavaScript library required:

1. Place `hero.mp4` in `public/videos/` directory (Vite serves `public/` files at root)
2. Use `<source src="/videos/hero.mp4" type="video/mp4" />` inside the `<video>` tag
3. Required attributes for autoplay: `autoPlay loop muted playsInline`
   - `autoPlay` -- start playing immediately
   - `loop` -- restart from beginning when ended (30s loop)
   - `muted` -- required for autoplay to work in all browsers
   - `playsInline` -- required for iOS Safari to play inline (not fullscreen)
4. H.264 codec ensures compatibility across all modern browsers (Chrome, Firefox, Safari, Edge)
